...
// hidden elements
let hiddenElements: any = null

// shape the issue
const shapeIssue = (issue: any) => {
  let context = ''
  let selector = ''
  ...
  return {
    ...
    code: issue.code,
    type: issue.type,
    typeCode: issueCodeMap[issue.type as keyof typeof issueCodeMap] || 0,
...
export const runA11y = async (options: any) => {
  // determine if valid issue
  const isIssueNotIgnored = (issue: any) => {
  ...
  // element test area
  const isElementInTestArea = (element: any) => {
    if (!rootElement) {
  ...
  const isElementOutsideHiddenArea = (element: any) => {
  ...
  const processIssues = (
    ...
    for (const is of issues) {
      ...
      if (scoreMap.has(issue.code)) {
        const [accessScore, ref] = scoreMap.get(issue.code) as any[]
  ...
  const runnerIssues = options.runners
  ...
  const issues = new Array(
    multiRunners
      ? runnerIssues[0].issues.length + runnerIssues[1].issues.length
      : runnerIssues[0].issues.length
  )
